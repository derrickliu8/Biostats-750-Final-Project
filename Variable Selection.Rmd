...
We observe that the mean square error from lasso (0.135445) is smaller than the
mean square error from ridge (0.2235973). 
...
intercept <- matrix(NA, k, 14)

for(j in 1:k){ 
  #best.fit=regsubsets(output~.,data=heart_dat[folds!=j,], nvmax =13) 
  forward_logregmodel <- glm(output ~., data = heart_dat[folds != j,], family = binomial) %>%
  stepAIC(direction = "forward", trace = FALSE)
  model_saver[j] <- length(forward_logregmodel$coefficients)   #saves the number of non zero coefficients
  intercept[j,] <- forward_logregmodel$coefficients[1:14] #looks at model coeffeicent
